...
def get_activations(root, model, batch_size=50, dims=2048, device='cpu',
                    num_workers=1):
    ...
    mean, std = compute_mean_and_std(ImageDataset, root=root, num_workers=0)
    transforms = TF.transforms.Compose([TF.transforms.ToTensor(),
                                        TF.transforms.Normalize(mean, std)])
    # transforms = TF.transforms.Compose([TF.transforms.ToTensor()])

    dataset = ImageDataset(root=root, transform=transforms)
...
def compute_statistics_of_path(path, model, batch_size, dims, device,
    ...
    m, s = calculate_activation_statistics(path, model, batch_size, dims, device, num_workers)
