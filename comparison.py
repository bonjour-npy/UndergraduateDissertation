...
def eval(args):
    ...
    generator_ipl = SG2Generator(args.ipl_ckpt, img_size=args.size,
                                 channel_multiplier=args.channel_multiplier, device=device)
    generator_ipl.freeze_layers()
    generator_ipl.eval()
    ...
    with torch.no_grad():
        ...
        sample_nada = \
            generator_nada(sample_w, input_is_latent=True, truncation=args.sample_truncation, randomize_noise=False)[
                0]
        sample_ipl = \
            generator_ipl(sample_w, input_is_latent=True, truncation=args.sample_truncation, randomize_noise=False)[
                0]
        sample_ipl_improved = \
            generator_ipl_improved(sample_w, input_is_latent=True, truncation=args.sample_truncation,
                                   randomize_noise=False)[0]

    grid_rows = int(args.n_sample)
    sample = torch.concat([sample_src, sample_nada, sample_ipl, sample_ipl_improved], dim=0)
