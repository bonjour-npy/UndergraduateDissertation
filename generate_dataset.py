...
def generate(args):
    ...
    # Add tqdm here
    for epoch in tqdm(range(args.epochs_generate)):
        ...
        for i in range(args.n_generate):
            ...
            image_tar = sample[i]
            image_src_path = os.path.join(dataset_dir_src, f"{args.src_label}_{(epoch * 10 + i):04d}.png")
            image_tar_path = os.path.join(dataset_dir_tar, f"{args.tar_label}_{(epoch * 10 + i):04d}.png")
